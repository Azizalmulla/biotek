        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-semibold text-black">Research Portal</h1>
                <p className="text-sm text-black/50">Population Analytics & De-identified Data</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
                <p className="text-3xl font-bold text-black">{stats.total_patients.toLocaleString()}</p>
                <p className="text-sm text-black/50 mt-1">Total Population</p>
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
                <p className="text-3xl font-bold text-green-600">{stats.risk_distribution[0]?.count || 0}</p>
                <p className="text-sm text-black/50 mt-1">Low Risk</p>
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
                <p className="text-3xl font-bold text-amber-600">{stats.risk_distribution[1]?.count || 0}</p>
                <p className="text-sm text-black/50 mt-1">Moderate Risk</p>
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
                <p className="text-3xl font-bold text-red-600">{stats.risk_distribution[2]?.count || 0}</p>
                <p className="text-sm text-black/50 mt-1">High Risk</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              {/* Age Distribution */}
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
                <h3 className="font-semibold text-black mb-4">Age Distribution</h3>
                <div className="space-y-3">
                  {stats.age_distribution.map((item) => (
                    <div key={item.range} className="flex items-center gap-4">
                      <span className="text-sm text-black/60 w-16">{item.range}</span>
                      <div className="flex-1 bg-black/10 rounded-full h-6 overflow-hidden">
                        <div 
                          className="bg-black h-full rounded-full"
                          style={{ width: `${(item.count / stats.total_patients) * 100 * 3}%` }}
                        />
                      </div>
                      <span className="text-sm font-medium text-black w-12">{item.count}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Disease Prevalence */}
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
                <h3 className="font-semibold text-black mb-4">Disease Prevalence</h3>
                <div className="space-y-3">
                  {stats.disease_prevalence.map((item) => (
                    <div key={item.disease} className="flex items-center gap-4">
                      <span className="text-sm text-black/60 flex-1 truncate">{item.disease}</span>
                      <div className="w-32 bg-black/10 rounded-full h-6 overflow-hidden">
                        <div 
                          className="bg-black/70 h-full rounded-full"
                          style={{ width: `${item.percentage * 2}%` }}
                        />
                      </div>
                      <span className="text-sm font-medium text-black w-16">{item.percentage}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Sex Distribution */}
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
              <h3 className="font-semibold text-black mb-4">Sex Distribution</h3>
              <div className="flex items-center gap-8">
                {stats.sex_distribution.map((item) => (
                  <div key={item.label} className="flex items-center gap-3">
                    ...
                    <span className="text-black/40">({((item.count / stats.total_patients) * 100).toFixed(1)}%)</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'cohort' && (
          ...
          >
            {/* Filters */}
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
              <h3 className="font-semibold text-black mb-6">Define Cohort Criteria</h3>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      className="w-24 px-3 py-2 border border-black/10 rounded-lg"
                      min={0}
                      max={120}
                    />
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-black/10 rounded-xl"
                  >
                ...
                {/* Disease Filter */}
                <div>
                  <label className="block text-sm font-medium text-black mb-2">Disease (optional)</label>
                  <select
                    onChange={(e) => {
                  ...
                  {cohortFilter.diseases.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {cohortFilter.diseases.map((d) => (
                        <span key={d} className="px-3 py-1 bg-black/10 text-black rounded-full text-sm flex items-center gap-2">
                          {d.replace(/_/g, ' ')}
                ...
                <button
                  onClick={buildCohort}
                  disabled={loading}
                  className="w-full py-3 bg-black text-white rounded-xl hover:bg-black/90 font-medium disabled:opacity-50"
                >
            ...
            {/* Results */}
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-black/10">
              <h3 className="font-semibold text-black mb-6">Cohort Results</h3>
              
              {cohortSize > 0 ? (
                <div className="text-center py-12">
                  <p className="text-6xl font-bold text-black">{cohortSize.toLocaleString()}</p>
                  <p className="text-black/50 mt-2">patients match criteria</p>
                  
                  <div className="mt-8 p-4 bg-black/5 rounded-xl text-left">
                    <p className="text-sm font-medium text-black mb-2">Cohort Definition:</p>
                  ...
                  <button
                    ...
                    className="mt-6 px-6 py-3 bg-black text-white rounded-xl hover:bg-black/90 font-medium"
                  >
                    Export This Cohort →
                  </button>
                </div>
          ...
          >
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-black/10">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 bg-black/5 rounded-xl flex items-center justify-center text-2xl">🔗</div>
                <div>
                  <h3 className="font-semibold text-black">Federated Learning Network</h3>
                  <p className="text-sm text-black/50">Train models across institutions without sharing raw data</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-6 mb-8">
                ...
                <div className="p-4 bg-black/5 rounded-xl border border-black/10">
                  ...
                  <p className="text-sm text-black/50">Total Samples</p>
                </div>
                <div className="p-4 bg-black/5 rounded-xl border border-black/10">
                  <p className="text-2xl font-bold text-black">v2.3</p>
                  <p className="text-sm text-black/50">Model Version</p>
                </div>
              </div>

              <div className="space-y-4">
                <h4 className="font-medium text-black">Network Nodes</h4>
                {[
                  ...
                  <div key={node.name} className="flex items-center justify-between p-4 bg-black/5 rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${
                        node.status === 'online' ? 'bg-green-500' : 
                        node.status === 'syncing' ? 'bg-amber-500 animate-pulse' : 'bg-black/40'
                      }`} />
        ...
        {activeTab === 'export' && (
          ...
          >
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-black/10">
              <h3 className="font-semibold text-black mb-2">Export De-identified Data</h3>
              ...
              <div className="space-y-4">
                <button
                  ...
                  className="w-full p-4 border border-black/10 rounded-xl hover:bg-black/5 flex items-center justify-between"
                >
                  <div className="flex items-center gap-4">
                    <span className="text-2xl">📄</span>
                    <div className="text-left">
                      <p className="font-medium text-black">CSV Export</p>
                      <p className="text-sm text-black/50">Spreadsheet-compatible format</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <span className="text-2xl">{ }</span>
                    <div className="text-left">
                      <p className="font-medium text-black">JSON Export</p>
                      <p className="text-sm text-black/50">Machine-readable structured data</p>
                    </div>
                  </div>
                  <span className="text-black">→</span>
                </button>
                ...
                >
                  <div className="flex items-center gap-4">
                    <span className="text-2xl">🔮</span>
                    <div className="text-left">
                      <p className="font-medium text-black">Synthetic Data</p>
                      <p className="text-sm text-black/50">AI-generated data matching real distributions</p>
                    </div>
                  </div>
                  <span className="text-black">→</span>
                </button>
